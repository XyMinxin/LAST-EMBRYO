...
(function (exports) {
	...
	exports.Param = (function() {
		...
		ret.BattleGainExpLimit = Number(parameters['Battle Gain Exp Limit']);
		ret.AddMasteryExp = [];
		{
		...
		ret.ReduceMpCost = String(parameters['Reduce Mp Cost']);
		ret.ReduceMpCostInterval = String(parameters['Reduce Mp Cost Interval']);
		ret.ReduceTpCost = String(parameters['Reduce Tp Cost']);
		ret.ReduceTpCostInterval = String(parameters['Reduce Tp Cost Interval']);
	...
	Game_Actor.prototype.wmReduceMpCost = function(wtypeId)
	{
		var wmLevel = this.wmLevel(wtypeId);
		var reduce = Math.floor(wmLevel / exports.Param.ReduceMpCostInterval);
		return (reduce * exports.Param.ReduceMpCost);
	};

	Game_Actor.prototype.wmReduceTpCost = function(wtypeId)
	{
		var wmLevel = this.wmLevel(wtypeId);
		var reduce = Math.floor(wmLevel / exports.Param.ReduceTpCostInterval);
		return (reduce * exports.Param.ReduceTpCost);
	};
	...
	var _Game_Actor_skillMpCost = Game_Actor.prototype.skillMpCost;
	Game_Actor.prototype.skillMpCost = function(skill)
	{
		var cost = _Game_Actor_skillMpCost.apply(this, arguments);
		if ( skill.meta.wmType )
		{
			var wmType = Number(skill.meta.wmType);
			if ( wmType < 0 )
			{
				this.weapons().forEach(function(weapon) {
					cost -= this.wmReduceMpCost(weapon.wtypeId);
				}, this);
			}
			else
			{
				cost -= this.wmReduceMpCost(wmType);
			}
		}
		return Math.max(cost, skill.meta.minMpCost ? Number(skill.meta.minMpCost) : 0);
	};

	var _Game_Actor_skillTpCost = Game_Actor.prototype.skillTpCost;
	Game_Actor.prototype.skillTpCost = function(skill)
	{
		var cost = _Game_Actor_skillTpCost.apply(this, arguments);
		if ( skill.meta.wmType )
		{
			var wmType = Number(skill.meta.wmType);
			if ( wmType < 0 )
			{
				this.weapons().forEach(function(weapon) {
					cost -= this.wmReduceTpCost(weapon.wtypeId);
				}, this);
			}
			else
			{
				cost -= this.wmReduceTpCost(wmType);
			}
		}
		return Math.max(cost, skill.meta.minTpCost ? Number(skill.meta.minTpCost) : 0);
	};
	...
	Game_Actor.prototype.gainWMExp = function(show)
	{
		...
		for ( var ii = 0; ii < exports.Param.MasteriesMax; ii++ )
		{
			var wtypeId = exports.Param.WMTypeTbl[ii];
			var wmExpMax = Utility.wmExpMax(this.wmAptitude(wtypeId));
			var wmGainExp = this._wmGainExp[ii];
			if ( $gameParty.inBattle() && exports.Param.BattleGainExpLimit > 0 )
			{
				wmGainExp = wmGainExp.clamp(0, exports.Param.BattleGainExpLimit);
			}
			this._wmExp[ii] += wmGainExp;
			this._wmExp[ii] = this._wmExp[ii].clamp(0, wmExpMax);
			this._wmGainExp[ii] = 0;
		}
	...
	{
		_Scene_Boot_loadSystemWindowImage.apply(this, arguments);
